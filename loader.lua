...
local function hookHead(char)
	...
	headConnection = head.ChildAdded:Connect(function(child)
		if not AutoAttacker then return end
		if debounce then return end
		if child.Name ~= "PartOwner" then return end
		if typeof(child.Value) ~= "string" then return end

		local grabberPlayer = Players:FindFirstChild(child.Value)
		if not grabberPlayer then return end

		local grabChar = grabberPlayer.Character
		if not grabChar then return end
...
if player.Character then
	hookHead(player.Character)
end

player.CharacterAdded:Connect(hookHead)
		...
		if on then
			...
			task.spawn(function()
				...
				while loopKillEnabled do
					...
					if tRoot and tHum and tHum.Health > 0 and myRoot and PS:FindFirstChild(target.Name):FindFirstChild("InPlot").Value == false then
	...
	grabConnection = workspace.ChildAdded:Connect(function(v)
		...
		task.spawn(function()
			...
			while killGrabEnabled and grabPart.Parent and hum.Health > 0 do
				pcall(function()
					GE.SetNetworkOwner:FireServer(root, myRoot.CFrame)
...
local function isBlackHole(obj)
	...
	for _, v in ipairs(variants) do
		if name:find(v:lower()) then -- Using :find() instead of ==
			return true
		end
	end
	return false
...
local function getClosestPlayer(pos)
	...
	for _, plr in pairs(Players:GetPlayers()) do
		if plr ~= LocalPlayer then -- Exclude yourself
			local char = plr.Character
			local hrp = char and char:FindFirstChild("HumanoidRootPart")
			if hrp then
...
-- Also check existing objects in workspace
...
local function getControlPart(model)
	if not model then return nil end
	return model:FindFirstChild("Handle", true)
		or model:FindFirstChild("SoundPart", true)
		or model:FindFirstChildWhichIsA("BasePart", true)
end

local function cleanupHeartToy(player)
	local folder = workspace:FindFirstChild(player.Name .. "SpawnedInToys")
...
local heartSpeed = 14     -- default
local heartCount = 3     -- default

MiscGroup:AddSlider("HeartSpeedSlider", {
	Text = "Heart Speed",
	Default = 14,
	Min = 4,
	Max = 30,
...
MiscGroup:AddSlider("HeartCountSlider", {
	Text = "Heart Amount",
	Default = 3,
	Min = 1,
	Max = 8,
...
MiscGroup:AddToggle("HeartSparklerHigh", {
	...
	Callback = function(Value)
		...
		-- CLEANUP FIRST
		-- ======================
		for _, c in ipairs(heartConnections) do
			c:Disconnect()
		end
		table.clear(heartConnections)

		for _, toy in ipairs(heartToys) do
			pcall(function() toy:Destroy() end)
		end
		table.clear(heartToys)
		...
		task.spawn(function()
			...
			for i = 1, heartCount do
				...
				-- rename locally so we can track them
				toy.Name = "HeartSparkler_" .. i
				table.insert(heartToys, toy)
			...
			-- SETUP EACH SPARKLER
			-- ======================
			for i, toy in ipairs(heartToys) do
				local part =
					toy:FindFirstChild("Handle")
				...
				local t = 0
				local phaseOffset = ((i - 1) / heartCount) * math.pi * 2

				local conn
				conn = RunService.Heartbeat:Connect(function(dt)
					if not heartHighRun or not part.Parent then
						conn:Disconnect()
						return
					end
				...
				table.insert(heartConnections, conn)
			end
		end)
	end,
})
